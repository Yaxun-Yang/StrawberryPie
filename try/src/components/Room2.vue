...
export default {
  ...
  methods: {
    ...
    async getRoom2Infomation () {
      const { data: res } = await this.$http.get('room2')
      console.log(res)
      if (res.meta.status !== 200) {
        return this.$message.error('获取卫生间信息失败')
      }
    ...
    async tempLightChange() {
      const { data: res } = await this.$http.put(
              `room2/templight/change/${this.value1}`
      )
      if (res.meta.status !== 200) {
    ...
    async switchChange2() {
      this.connectDev()
      const { data: res } = await this.$http.put(
              `room2/lightcon/change/${this.value2}`
      )
      if (res.meta.status !== 200) {
        return this.$message.error('修改失败，请重新输入')
      }
